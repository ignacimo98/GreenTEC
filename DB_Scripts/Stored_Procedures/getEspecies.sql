...
CREATE PROCEDURE ObtenerEspeciesDeCadena AS
  WITH CadenaAlimenticia
	AS (SELECT IdDepredador AS Id FROM GREEN_TEC.Alimentacion
	UNION
	SELECT IdPresa AS Id FROM GREEN_TEC.Alimentacion)
	SELECT IdEspecie, IdTipoEspecie AS IdTipo, NombreCientifico, NombreVulgar FROM GREEN_TEC.Especie
