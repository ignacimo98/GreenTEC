...
BEGIN TRAN
  BEGIN TRY
    PRINT 'Insercion 1';
    INSERT INTO [GREEN_TEC].[GREEN_TEC].[Vehiculo] ([Tipo], [Matricula]) VALUES ('PickUp', 'KBP798');
    PRINT 'Insercion 2';
    INSERT INTO [GREEN_TEC].[GREEN_TEC].[Tour] ([Nombre], [Descripcion]) VALUES ('Avistamiento de felinos', 'Caminata por los bosques para alimentar y convivir con felinos');
    PRINT 'Insercion 3';
    INSERT INTO [GREEN_TEC].[GREEN_TEC].[ParqueNacional] ([Nombre], [FechaDeclaracion]) VALUES ('Parque Nacional Marino Ballena', '1980-11-10');
    PRINT 'Insercion 4';
    INSERT INTO [GREEN_TEC].[GREEN_TEC].[Visitante] ([Cedula], [Nombre], [Domicilio], [Profesion]) VALUES (280489389, 'Jerry Arias', 'Cartago, Costa Rica', 'Estudiante');
    COMMIT TRAN
...
AS
  BEGIN TRAN
    BEGIN TRY
      ...
      INSERT INTO [GREEN_TEC].[GREEN_TEC].[CaracteristicaEspecie] ([Nombre], [Valor]) VALUES (6/0, 6); --error
      PRINT 'Insertando en CaracteristicaPersonal' 
      INSERT INTO [GREEN_TEC].[GREEN_TEC].[CaracteristicaPersonal] ([Nombre], [Valor]) VALUES (6/0, 6); --error
      COMMIT TRAN
      PRINT 'Transaccion comitteada'
...
AS
  BEGIN TRAN
    BEGIN TRY
      ...
      PRINT 'Insertando en CaracteristicaEspecie'
      INSERT INTO [GREEN_TEC].[GREEN_TEC].[CaracteristicaEspecie] ([Nombre], [Valor]) VALUES ('Color', 'Rojo'); --no error
      PRINT 'Insertando en CaracteristicaPersonal' 
      INSERT INTO [GREEN_TEC].[GREEN_TEC].[CaracteristicaPersonal] ([Nombre], [Valor]) VALUES ('Altura', '1.54'); --no error
      COMMIT TRAN
      PRINT 'Transaccion comitteada'
...
BEGIN
      ...
      PRINT 'Leyendo XML'
      DECLARE @xml XML
      DECLARE @SQL NVARCHAR(1000) = 'SET @xml = (SELECT * FROM OPENROWSET (BULK ''' + @Ruta + ''', SINGLE_CLOB) AS XmlData)'
      ...
      SELECT
      row.value('(Nombre/text())[1]', 'VARCHAR(100)') AS Nombre,
      row.value('(Valor/text())[1]', 'VARCHAR(100)') AS Valor
      FROM
      @xml.nodes('/data/row') AS Listing(row);
      PRINT 'Finalizado'
...
'SET @xml = (SELECT * FROM OPENROWSET (BULK ''' + 'M:\MSSQL14.MSSQLSERVER\MSSQL\DATA\CaracteristicasEspecie.xml' + ''', SINGLE_CLOB) AS XmlData)'
...
USING (VALUES ('Comportamiento','Agresivo'), ('Comportamiento', 'Timido'))  
